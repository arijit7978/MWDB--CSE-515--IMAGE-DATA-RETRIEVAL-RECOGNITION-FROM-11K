...
def labeled_PPR(combined_df,metadata_path,label):
    labeled_imgs = metadata_labels(metadata_path, label)
...
def labelling_unlabeled_data(dorsal_ppr,palmar_ppr,unlabeled_folder_path):
    result = []
    for image in os.listdir(unlabeled_folder_path):
        dorsalindex = -1
        palmarindex = -1
        ...
        if dorsalindex < palmarindex:
            result.append([image, 'dorsal'])
        elif dorsalindex == palmarindex:
            result.append(
                ([image, 'dorsal' if dorsal_ppr[dorsalindex][1] > palmar_ppr[palmarindex][1] else 'palmar']))
        else:
            result.append([image, 'palmar'])
