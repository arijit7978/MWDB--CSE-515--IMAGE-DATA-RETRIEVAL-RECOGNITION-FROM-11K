...
def train_test_data_dcsn(labeled_folder_path,unlabeled_folder_path,labeled_metadata_path):
    cm(unlabeled_folder_path, 'cm_set2_unlabelled')
    cm(labeled_folder_path, 'cm_set2')
    if (os.path.exists('train_data.csv')):
        os.remove('train_data.csv')

    # train_data on features

    data = pd.read_csv(labeled_metadata_path)
    ...
    for row in test_data:
        print("imagename:%s prediction:%s" %
              (row[0], predict_dcsn(row[1:], my_tree)))
...
def cross_validation(x, y, xtest, learning_rate, epoch, validations, labels):
    ...
    for valid in range(validations):
        ...
        for item in y_pred:
            if item == [1, 0]:
                # print("dorsal")
                classification.append("dorsal")
            else:
                # print("palmar")
                classification.append("palmar")
...
def train_test_data_svm(labeled_folder_path,unlabeled_folder_path,labeled_metadata_path):
    hog(unlabeled_folder_path, 'hog_set2_unlabelled')
    hog(labeled_folder_path, 'hog_set2')
    if (os.path.exists('train_data(set2).csv')):
        os.remove('train_data(set2).csv')
    if (os.path.exists('test_data(unlabelled_set2).csv')):
        os.remove('test_data(unlabelled_set2).csv')

    # test data on latent semantics
    dfdf = pd.read_csv('hog_set2_unlabelled.csv', sep=',', header=None)
...
def read_argument(argv):
    ...
    parser.add_argument("-c", "--classifier", type=str, help="The classifier")
...
if __name__ == '__main__':
    ...
    if classifier == "svm":
        train_test_data_svm(labeled_folder_path,unlabeled_folder_path,labeled_metadata_path)
        svm()
    elif classifier == "dcsn":
        train_test_data_dcsn(labeled_folder_path,unlabeled_folder_path,labeled_metadata_path)

    else:
        main(labeled_folder_path,unlabeled_folder_path,labeled_metadata_path,labeled_set,unlabeled_set)
